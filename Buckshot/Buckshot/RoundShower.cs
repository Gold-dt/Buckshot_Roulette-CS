...
namespace Buckshot
{
    internal class RoundShower
    {
        public void RDisplay(int Round,int MaxRound,int LastRound)
        {
        ...
        public void RStarterDisplay(int Round, int MaxRound, int LastRound)
        {
            int margin = 10 - 5 - Round.ToString().Length - MaxRound.ToString().Length;
            string tab = "\t\t\t\t\t\t      ";
            ...
            // Első kijelzés
            Console.WriteLine("\n\n");
            Console.WriteLine($"{tab}╭{"─".PadLeft(10, '─')}╮\n{tab}│   {LastRound}/{0}{"".PadLeft(margin, ' ')} │\n{tab}╰{"─".PadLeft(10, '─')}╯");
            Thread.Sleep(1500);
            Console.Clear();
